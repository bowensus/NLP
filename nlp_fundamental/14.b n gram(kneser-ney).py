...
class Language_Model:
    ...
    def process_line(self, line):
        ...
        for token in tokens:
            ...
            current = self.bigram.get(profix, {})
            current[token] = current.get(token, 0) + 1
            self.bigram[profix] = current
    ...
    def get_prob(self, token, context='', method="unigram", smoothing="kneser-ney"):
        if method == "unigram":
            return self.unigram.get(token, self.unigram.get("__UNKNOWN", 0))
        elif method == "bigram":
            # eg: token == "go", context == ["I", "want", "to"], context[-1] == "to"
            if smoothing == "keneser-ney":
                unidist = self.kn
            else:
                unidist = self.unigram
            bigram = self.bigram.get(context[-1], self.bigram.get("__UNKNOWN", {}))
            bigram_p = bigram.get(token, bigram.get("__UNKNOWN", 0))
            weight = bigram["__DISCOUNT"]
            unigram_p = self.unigram.get(token, unidist.get("__UNKNOWN", 0))
            # weight, usually 0 ~ 1
            p = bigram_p + weight * unigram_p
            return p
        else:
            print("Not Implement")
            return 0
    ...
    def nextlikely(self, k=1, current='', method="unigram"):
        ...
        if method == "unigram":
            ...
            dist = self.bigram.get(current, self.bigram.get("__UNKNOWN", {}))
    ...
    def compute_prob_line(self, line, method="unigram", smoothing="kneser-ney"):
        tokens = ["__START"] + word_tokenize(line) + ["__END"]
        acc = 0
    ...
    def compute_probability(self, filenames=[], method="unigram", smoothing="kneser-ney"):
        if filenames == []:
    ...
    def compute_preplexity(self, filenames=[], method="unigram", smoothing="kneser-ney"):
    ...
    def make_unknowns(self, known=2):
        ...
        for (key, all_next) in list(self.bigram.items()):
            ...
            if isknown == 0:
                del self.bigram[key]
                current = self.bigram.get("__UNKNOWN", {})
    ...
    def discount(self, discount=0.75):
        # if times < isknown, valuse - discount
        self.bigram = {key: {k: v - discount for (k, v) in all_next.items()} for (key, all_next) in self.bigram.items()}
        for key in self.bigram.keys():
            lamb = len(self.bigram[key])
            self.bigram[key]["__DISCOUNT"] = lamb * discount

        for (key, all_next) in self.bigram.items():
            for k in all_next.keys():
                self.kn[k] = self.kn.get(k, 0) + 1
                

max_files = 5
model = Language_Model(training_dir, training_files[:max_files])
model.generate(method="bigram")
model.compute_preplexity()
model.compute_preplexity(method="bigram")
